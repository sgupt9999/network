...
INTERFACE1="eth0"
INTERFACE2="eth1"
IP1=172.31.31.131/32
IP2=172.31.27.130/32
ROUTER1=172.31.16.1
ROUTER2=172.31.16.1
#INTERFACE2_UP="yes"
INTERFACE2_UP="no"
...
yum install NetworkManager-dispatcher-routing-rules-1.12.0-8.el7_6.noarch.rpm -y
systemctl enable --now NetworkManager-dispatcher.service 
rm -rf NetworkManager-dispatcher-routing-rules-1.12.0-8.el7_6.noarch.rpm

# Bring the 2nd network card if not up
if [[ $INTERFACE2_UP == "no" ]]
then
	nmcli connection delete test
	nmcli connection add con-name test type ethernet ifname $INTERFACE2 autoconnect true
